...
public class List_inChainOfNodes {
  ...
  /**
   * @return a string representation of this list, format: # elements
   *         [element0,element1,element2,]
   */
  public String toString() {
    String stringRep = size() + " elements [";

    for (Node node = headSentinel.getNextNode(); node != tailSentinel; node = node.getNextNode())
  ...
  public String toStringReverse() {
    ...
    for (Node node = tailSentinel.getPreviousNode(); node != headSentinel; node = node.getPreviousNode())
      stringRep += node.getCargo() + "`";
    return stringRep + "]";
  }
  ...
  public boolean addAsHead(Object val) {
    Node toInsert = new Node(val, headSentinel, headSentinel.getNextNode());
    headSentinel.getNextNode().setPreviousNode(toInsert);
    headSentinel.setNextNode(toInsert);
    return true;
  }

  /**
   * @return a reference to the node before the node at @index
   */
  private Node getNodeBefore(int index) {
    /*
     * iterate through the chain, up to the node that holds a reference to the
     * desired node
     */

    Node node;
    int upTo; // comma operator precludes declaration in FOR
    for (upTo = 0, node = headSentinel; upTo < index; upTo++, node = node.getNextNode())
  ...
  public boolean add(int index, Object value) {
    ...
    Node newNode = new Node(value, beforeNew, afterNew);
